...
// Carga la vista del Contenedor principal
	...
	if(!file_exists($Path[src].$inc)){				
		...
		$menu_opc = array(	
					...
					,LINK_OPC61		=> '../site/?m='.$var[ADMIN].'&s='.$var[XLS]
					,txt_opc62		=> $dic[inplant][xls_lista_menu]
					,LINK_OPC62		=> '../site/?m='.$var[ADMIN].'&s='.$var[XLS_LISTA]
					// Administración
					,txt_opc7 		=> $dic[general][admin]
					,img_opc7		=> $var[menu_opc7]
					,LINK_OPC7		=> '#'									
					,txt_opc71		=> $dic[admin][sincronizar_menu]
					,LINK_OPC71		=> '../site/?m='.$var[ADMIN].'&s='.$var[SINCRONIZACION]
					,txt_opc72		=> $dic[admin][alta_usuario]
					,LINK_OPC72		=> '../site/?m='.$var[ADMIN].'&s='.$var[ALTA_USUARIO]
					,txt_opc73		=> $dic[admin][sincronizar_empresa]
					,LINK_OPC73		=> '../site/?m='.$var[ADMIN].'&s='.$var[SINCRONIZACION_EMPRESAS]
				);
